...
def canvass():
    ...
    # Option to save the drawn canvas
    saveCanvasBtn = ColorRect(1000, 0, 100, 100, (150, 150, 150), 'Save')
    ...
    while True:
        ...
        if upFingers:
            ...
            if upFingers[1] and not whiteBoard.isOver(x, y):
                ...
                # Save button
                if saveCanvasBtn.isOver(x, y) and not coolingCounter:
                    coolingCounter = 10
                    saveCanvasBtn.alpha = 0
                    # Save the canvas as an image file (e.g., PNG)
                    cv2.imwrite(f'img{save_counter}.png', canvas)
                    save_counter +=1
                else:
                    saveCanvasBtn.alpha = 0.5
        ...
        if not hideBoard:
            ...
            # Moving the drawing to the main image
        ...
        # Save button
        saveCanvasBtn.drawRect(frame)
        cv2.rectangle(frame, (saveCanvasBtn.x, saveCanvasBtn.y), (saveCanvasBtn.x + saveCanvasBtn.w, saveCanvasBtn.y + saveCanvasBtn.h),
                      (255, 255, 255), 2)
